...
@router.post("/token", response_model=ApiResponse, dependencies=[Depends(get_db)])
def token(request_data: OAuth2PasswordRequest):
    """
    用户名+密码登录
    """
    grant = PasswordGrant(request_data)
    return success(grant.respond())


@router.post("/cellphone/token", response_model=ApiResponse, dependencies=[Depends(get_db)])
